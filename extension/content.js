  ...
  let sponsorSegments = [];
  let sponsorSkipEnabled = false;
  let isSkipping = false;
  let beamStartVideoTime = 0;
  let beamStartWallTime = Date.now();

  const getVideoId = () => {
    try { return new URL(window.location.href).searchParams.get('v'); } catch { return null; }
  };

  const fetchSponsorSegments = async (videoId) => {
    sponsorSegments = [];
    if (!videoId) return;
    try {
      ...
      if (res.ok) {
        sponsorSegments = (await res.json()).map(s => ({ start: s.segment[0], end: s.segment[1] }));
      }
    } catch {}
  };

  const getEstimatedServerTime = () => beamStartVideoTime + (Date.now() - beamStartWallTime) / 1000;

  const getCurrentSponsorSeg = (time) =>
    sponsorSegments.find(s => time >= s.start && time < s.end) || null;

  const beamSeekTo = async (endTime) => {
    beamStartVideoTime = endTime;
    beamStartWallTime = Date.now();
    overlayText.innerText = 'Skipping sponsor...';
    try {
      await fetch(`${SERVER}/control`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'seek_to', time: endTime })
      });
    } catch {}
    overlayText.innerText = 'Playing on Server';
  };

  const skipSponsor = (seg) => {
  ...
  // SponsorBlock skip button in overlay (for beaming)
  const sbSkipOverlayBtn = createOverlayBtn('Skip Sponsor', () => {
    const t = getEstimatedServerTime();
    const seg = getCurrentSponsorSeg(t) || sponsorSegments.find(s => s.end > t) || null;
    if (seg) skipSponsor(seg);
  });
  sbSkipOverlayBtn.style.backgroundColor = '#2d7d46';
  sbSkipOverlayBtn.onmouseover = () => sbSkipOverlayBtn.style.backgroundColor = '#3a9e5a';
  sbSkipOverlayBtn.onmouseout = () => sbSkipOverlayBtn.style.backgroundColor = '#2d7d46';
  ...
  overlayControls.appendChild(sbSkipOverlayBtn);
  ...
  // SponsorBlock panel controls
  const sbToggleBtn = document.createElement('button');
  sbToggleBtn.style.padding = '6px 8px';
  sbToggleBtn.style.color = '#fff';
  sbToggleBtn.style.border = 'none';
  sbToggleBtn.style.borderRadius = '4px';
  sbToggleBtn.style.cursor = 'pointer';
  sbToggleBtn.style.fontSize = '12px';
  sbToggleBtn.style.fontWeight = 'bold';

  const updateSbToggle = () => {
    sbToggleBtn.innerText = `SponsorBlock: ${sponsorSkipEnabled ? 'ON' : 'OFF'}`;
    sbToggleBtn.style.backgroundColor = sponsorSkipEnabled ? '#2d7d46' : '#555';
    sbToggleBtn.onmouseover = () => sbToggleBtn.style.backgroundColor = sponsorSkipEnabled ? '#3a9e5a' : '#666';
    sbToggleBtn.onmouseout = () => sbToggleBtn.style.backgroundColor = sponsorSkipEnabled ? '#2d7d46' : '#555';
  };
  sbToggleBtn.onclick = () => { sponsorSkipEnabled = !sponsorSkipEnabled; updateSbToggle(); };
  updateSbToggle();

  const sbSkipLocalBtn = createSmallBtn('Skip Sponsor', () => {
    const video = document.querySelector('video');
    if (!video) return;
    const seg = getCurrentSponsorSeg(video.currentTime);
    if (seg) skipSponsor(seg);
  });
  sbSkipLocalBtn.style.display = 'none';
  sbSkipLocalBtn.style.backgroundColor = '#2d7d46';
  sbSkipLocalBtn.onmouseover = () => sbSkipLocalBtn.style.backgroundColor = '#3a9e5a';
  sbSkipLocalBtn.onmouseout = () => sbSkipLocalBtn.style.backgroundColor = '#2d7d46';

  container.appendChild(title);
  container.appendChild(qualitySelect);
  container.appendChild(beamBtn);
  container.appendChild(sbToggleBtn);
  container.appendChild(sbSkipLocalBtn);
  ...
  const attachVideoListener = () => {
    const video = document.querySelector('video');
    if (!video || video._sbAttached) return;
    video._sbAttached = true;
    video.addEventListener('timeupdate', () => {
      if (!sponsorSegments.length) return;
      const seg = getCurrentSponsorSeg(video.currentTime);
      sbSkipLocalBtn.style.display = (seg && !isBeaming) ? 'block' : 'none';
      if (sponsorSkipEnabled && !isSkipping && !isBeaming && seg) skipSponsor(seg);
    });
  };

  setInterval(attachVideoListener, 1000);
